...
export default function AdminChangeReservationStatusModal({
  status,
  id,
  updateReserv,
}: {
  ...
  const [reservationStatus, setReservationStatus] = useState("Upcoming");
  useEffect(() => {
    if (status) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    setReservationStatus(data?.type);
    const res = await updateReserv({ id, status: data.type });
    if (res) {
      handleClick();
    }
    // setClose(true);
  ...
  // Step 3: Function to trigger the button click
  const handleClick = () => {
    if (buttonRef.current) {
      buttonRef.current.click(); // Step 3: Access the current property and call click()
    }
  };
